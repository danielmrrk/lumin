...
class OutputLayer(Layer):
    def __init__(self, linear: Linear, activation: ActivationType = None, norm: NormalizationType = None):
        if activation is None:
            self.linear = linear
            self.activation = None
            self.norm = None
        else:
            super().__init__(linear, activation, norm)
    ...
    def backprop(self, grad_output: np.array, optimizer: Optimizer, kwargs: dict = None) -> np.array:
        if self.activation is None:
            ...
            return grad_output
        return super().backprop(grad_output, optimizer, kwargs)
